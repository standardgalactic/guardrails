...
Let $\mu_{\mathrm{bio}}$ be the probability measure on $\mathcal{F}_{\mathrm{all}}$ induced
by entropy-weighted dynamics.  Then for any $\epsilon > 0$, there exists a subset
$\mathcal{F}_\epsilon \subset \mathcal{F}_{\mathrm{all}}$ such that
\[
\mu_{\mathrm{bio}}(\mathcal{F}_\epsilon) \ge 1 - \epsilon,
\qquad
\dim(\mathcal{F}_\epsilon) \ll \dim(\mathcal{F}_{\mathrm{all}}).
...
\begin{proof}[Sketch]
The entropy weight $w_k = e^{-\lambda S}$ assigns exponentially small probability to
trajectories with $S > S_{\max}$.  Since $\mathcal{T}_{\mathrm{bio}}$ is contained in a
finite-dimensional manifold $\mathcal{M}_{\mathrm{bio}}$ (Proposition above), the pushforward
measure $\mu_{\mathrm{bio}}$ is supported on a subset of $\mathcal{F}_{\mathrm{all}}$ of
dimension at most $\dim(\mathcal{M}_{\mathrm{bio}})$.  Taking $\mathcal{F}_\epsilon$ to
be the set of functions realisable by trajectories with $S \le S_{\max} + \delta(\epsilon)$
for appropriate $\delta(\epsilon) \to 0$ as $\epsilon \to 0$ yields the claim.
\end{proof}
